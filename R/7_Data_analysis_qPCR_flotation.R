...
sdt%>%
  ...
  theme(text = element_text(size=16))+
  annotation_logticks(sides = "l")+
  coord_cartesian(ylim = c(100, 1000000))-> oocyst38
...
sdt%>%
  ...
  theme(text = element_text(size=16))+
  annotation_logticks(sides = "l")+
  coord_cartesian(ylim = c(100, 1000000))-> dna38
...
elod%>%
  ...
  scale_x_log10(name = "log10 Genome copies/µL gDNA dpi 3 (qPCR)", 
                breaks = scales::trans_breaks("log10", function(x) 10^x),
                labels = scales::trans_format("log10", scales::math_format(10^.x)))+
  scale_y_log10(name = "log10 Oocyst per gram feces dpi 8 (Flotation)", 
                breaks = scales::trans_breaks("log10", function(x) 10^x),
                labels = scales::trans_format("log10", scales::math_format(10^.x)))+
  geom_jitter(shape=21, position=position_jitter(0.2), size=5, aes(fill= EH_ID), color= "black")+
  labs(tag= "F)")+
  theme_bw()+
  theme(text = element_text(size=16))+
  stat_cor(label.x = 2.25, label.y = 3.5, aes(label= paste(..rr.label.., ..p.label.., sep= "~`,`~"))) +
  stat_regline_equation(label.x = 2.25, label.y = 4.0)+
  stat_cor(label.x = 2.25,  label.y = 3.75,method = "spearman")+
...
grid.arrange(oocyst36, dna36, elopp, oocyst38, dna38, elodp, ncol= 3, nrow= 2)
dev.off()
...
pdf(file = "~/AA_Microbiome/Figures/Oocysts_qPCR_Manuscript/Figure_4.1.pdf", width = 10, height = 10)
grid.arrange(oocyst38, dna38, elodp, widths = c(1, 1), layout_matrix = rbind(c(1, 2), c(3, 3)))
dev.off()
